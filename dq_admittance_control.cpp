#include <dq_admittance_control.h>
#include <cmath>
#include <math.h>
...
//                          		INIT										//
//------------------------------------------------------------------------------//
bool AdmittanceController::init(  hardware_interface::RobotHW* robot_hw, 
	...
	cartesian_stiffness_.bottomRightCorner(3, 3) 	<< 	300*Eigen::Matrix3d::Identity();
	cartesian_damping_.topLeftCorner(3, 3) 			<< 	2.0 * sqrt(1000)*Eigen::Matrix3d::Identity();	
	cartesian_damping_.bottomRightCorner(3, 3) 		<< 	2.0 * sqrt(300)*Eigen::Matrix3d::Identity();
...
void DQ_Admittance_Control::starting(const ros::Time& /*time*/) {
	...
	position_i_ = initial_transform.translation();
    ...
    r = r1*r2*r3;
    t = DQ([0 position_i_]);
    pose = r+0.5*DQ.E*t*r; %dq representing pose
...
void DQ_Admittance_Control::update( const ros::Time& /*time*/,
	...
	Eigen::Matrix<double, 8, 1> error;  	// pose error [8x1]
	Eigen::Matrix<double, 8, 1> derror; 	// pose vel. error [8x1]
	Eigen::Matrix<double, 8, 7> ja;			// pose Jacobian [8x6]
	Eigen::Matrix<double, 8, 7> ja_dot;		// derivative of pose jacobian [8x6]
    Eigen::Matrix<double, 8, 1> pose;       // current pose
	Eigen::Matrix<double, 8, 1> dpose;		// current pose velocity
